...
This directory provides **6700+ high-quality SVG icons** across three libraries that can be directly embedded into SVG files generated by PPT Goldwind.

These icons are bundled vector assets. They are not generated by `image_gen.py` and do not use image models such as `gpt-image-2` or `qwen-image-2.0-pro`.
...
- Standard PPT icons: use this SVG library and `finalize_svg.py`.
- Raster image generation: use `scripts/image_gen.py` only for backgrounds, photos, illustrations, decorative patterns, or large diagram-style images.
- Custom AI-generated icons: allowed only when explicitly requested or when no approved library icon fits after search; store them as image assets and document the active image backend/model.
