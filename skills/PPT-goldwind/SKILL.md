...
description: >
  ...
  "制作演示文稿", or mentions "PPT-goldwind".
---
...
**Core Pipeline**: `Source Document → Create Project → Template Option → Strategist → [Proactive gpt-image-2 Image_Generator] → Executor → Native Goldwind PPTX or SVG Post-processing → Export`
...
>
> - `PPT-goldwind` is a repository-specific workflow skill, not a general application scaffold
...
> 1. PPT样式参考：请发送一份您过往汇报中使用的PPT或历史模板文件，我将优先读取PPT原生母版、版式、占位符、坐标和主题样式，并结合页面视觉参考抽象出可复用模板风格；也可以选择使用本技能包中自带的金风通用模板。
...
> 4. Before entering the normal PPT Goldwind pipeline, verify whether the following intake items are sufficiently covered:
...
- For `金风通用模板`, default正文页信息密度要面向真实100寸投影汇报：每页优先承载10-18个有效业务信息单元，至少覆盖“结论/指标/动作/责任/风险/下一步”等多个维度；正文可使用约6.5-10.5pt的小字号。高密度不等于卡片墙：每页必须按内容选择版式，优先在 `process_flow`、`timeline`、`risk_matrix`、`kpi_dashboard`、`comparison_table`、`image_explain`、`section_summary`、`dense_cards` 中选择；连续正文页不得全部使用 `dense_cards`。只有资料确实不足、用户明确要求留白页，或该页是封面/目录/章节停顿页时，才允许少于8个正文信息点。
- 提高密度时必须同步控制可读性和版式多样性：缩小字号、压缩灰色导语带、减少装饰留白、保持模块间距；任何标题重复、文字越界、图片压文字、表格压文字、模块堆叠或三页以上连续卡片墙都是阻塞缺陷。若信息点不足，先从用户素材中继续归纳二级要点、数据口径、检查项、责任项、风险项和追踪项，而不是把少量要点铺成稀疏页面。
- After building a native Goldwind PPTX, run `python3 ${SKILL_DIR}/scripts/goldwind_native_check.py <output.pptx>`, `python3 ${SKILL_DIR}/scripts/pptx_visibility_check.py <output.pptx>`, and for ordinary reporting decks `python3 ${SKILL_DIR}/scripts/goldwind_density_check.py <output.pptx>`. Any failure is blocking. Skip the density check only when the user explicitly requests a sparse executive/hero-style deck or the source material is too limited and that limitation is stated in the final answer.
...
| `${SKILL_DIR}/scripts/image_gen.py` | AI image generation; defaults to `image2` / `gpt-image-2` through the Codex provider config |
| `${SKILL_DIR}/scripts/api_image_generate.py` | Full low-level `gpt-image-2` generation/edit/reference-image tool copied from `api-image` |
...
| `${SKILL_DIR}/scripts/goldwind_density_check.py` | Goldwind reporting-density gate check for sparse content pages and hollow image pages |
...
- `image_gen.py` is for raster image assets such as backgrounds, photos, illustrations, decorative patterns, large diagram-style images, visually rich process diagrams, technical architecture visuals, and table-to-infographic treatments. It is not the default icon pipeline.
- If a user explicitly requests a custom AI-generated icon or a required metaphor cannot be found after searching the approved icon library, treat it as an image asset, record it in the Image Resource List, and document the active backend/model from `python3 ${SKILL_DIR}/scripts/image_gen.py --list-backends` or the runtime `IMAGE_BACKEND` / provider model variables. The default model is `gpt-image-2` through the `image2` backend.

## Proactive gpt-image-2 Asset Policy

- After the Eight Confirmations are approved, the Strategist must proactively decide where `gpt-image-2` raster assets would improve the deck. The user does not need to explicitly request image generation for every image-worthy slide.
- Prefer `gpt-image-2` for high-impact raster assets: cover backgrounds, chapter backgrounds, scenario illustrations, product/engineering concept visuals, technical mechanism diagrams, architecture diagrams, process flow visuals, risk/control maps, matrix-style frameworks, table-to-infographic conversions, and large explanatory diagrams.
- For dense business tables, flowcharts, architecture maps, or system diagrams, do not default to plain text tables when a visual treatment would improve comprehension. Add an Image Resource List item with `Status = Pending generation`, then generate it before Executor starts.
- Generated image briefs must be content-rich. Every generated image item must specify concrete business subject, named modules/stages, domain objects, relationships, status signals, and intended overlay labels/KPIs. Generic prompts such as "科技背景", "流程框架", "empty zones", "spacious empty zones", or "clean label zones" are forbidden because they create empty shell images.
- If the image-worthy slide lacks enough concrete content to make a full visual, ask the user one concise clarification before generation: "这张图需要重点呈现哪些对象、阶段、指标或业务场景？" Do not silently generate a hollow framework.
- Keep data accuracy under AI control: exact numbers, labels, tables, and legends should remain editable SVG/PPT text whenever precision matters. Use `gpt-image-2` for business scene detail, module shapes, object relationships, status patterns, and visual hierarchy; overlay exact data and labels in editable PPT/SVG when needed.
- Prompts for PPT image assets must avoid fake readable text, pseudo letters, UI gibberish, fake chart values, watermarks, and logos. Do not over-constrain the image into blank placeholders; allow rich visual details such as icons, lines, panels, device views, maps, wind turbines, data traces, matrix cells, progress states, and scenario objects, while final Chinese labels and exact numbers stay editable.
- For `金风通用模板`, image prompts MUST follow the native Goldwind slide style: mostly white or very light gray background, thin navy/teal linework, subtle pale-gray panels, restrained teal accents, clean engineering information-graphic composition, no dark command-center dashboard look, no neon glow, no cinematic black background, no full-screen sci-fi UI.
- For branded, factual, current, or product-specific visuals, collect references or research evidence first. If references are needed for generation/editing, use `${SKILL_DIR}/scripts/api_image_generate.py` with `--image` inputs; otherwise use the unified `${SKILL_DIR}/scripts/image_gen.py`.
- Do not generate raster images merely to replace simple vector icons, bullets, basic KPI cards, or standard charts that the bundled SVG/chart libraries can render cleanly.
...
12. For Goldwind正文页, plan and generate a high-density but readable 100-inch-projector layout: default to 10-18 information units per content slide, but do not equate density with cards. Each page in `deck_spec.json` should set a `layout` value such as `process_flow`, `timeline`, `risk_matrix`, `kpi_dashboard`, `comparison_table`, `image_explain`, `section_summary`, or `dense_cards`. Use `dense_cards` only when the content is genuinely a flat list of independent items; process, schedule, risk, KPI, comparison, and architecture pages must use their corresponding visual structure. Only split a slide when the text would overlap or become unreadable after adaptive sizing.
...
14. Run `goldwind_native_check.py`, `pptx_visibility_check.py`, and for ordinary reporting decks `goldwind_density_check.py` on the final native PPTX; any missing TOC left image, duplicate title, unfilled title placeholder, top-left horizontal copyright text, missing media, wrong layout binding, sparse content slide, or image page without editable overlay/detail text is a blocking failure.
...
> **Trigger condition**: Image approach includes user-requested AI generation, or the Strategist identifies any slide where a `gpt-image-2` raster asset would materially improve communication. If no image generation is needed, skip directly to Step 6 (Step 6 GATE must still be satisfied).
...
3. Generate images with the default `image2` / `gpt-image-2` backend:
   ```bash
   python3 ${SKILL_DIR}/scripts/image_gen.py "prompt" --aspect_ratio 16:9 --image_size 2K -o <project_path>/images --filename <asset_name>
   ```
4. For reference-image generation, localized edits, or composition/style transfer, use the low-level provider tool:
   ```bash
   python3 ${SKILL_DIR}/scripts/api_image_generate.py --prompt "prompt" --image "<reference.png>" --image-role "style/reference role" --size 2048x1152 --quality high --out "<project_path>/images/<asset_name>.png"
   ```
5. Before generation, reject any prompt/brief that lacks concrete business content. If the brief only states a style or generic chart type, ask the user: `这张图需要重点呈现哪些对象、阶段、指标或业务场景？`
...
```bash
python3 ${SKILL_DIR}/scripts/goldwind_native_check.py <project_path>/exports/<native_output>.pptx
python3 ${SKILL_DIR}/scripts/pptx_visibility_check.py <project_path>/exports/<native_output>.pptx
python3 ${SKILL_DIR}/scripts/goldwind_density_check.py <project_path>/exports/<native_output>.pptx
```
