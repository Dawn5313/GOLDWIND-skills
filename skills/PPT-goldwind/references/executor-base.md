...
- Goldwind正文页默认服务100寸投影汇报，优先提高单页信息密度：一页应尽量承载10-18个有效业务信息单元，正文小字号可下探到约6.5-10.5pt。高密度必须匹配内容结构，而不是反复使用卡片墙：流程用泳道/链路，计划用时间线，风险用矩阵，指标用看板，比较用表格，图片页用图解标注，只有平铺独立事项才用卡片。不要把可合并的内容拆成只有2-5条要点的稀疏页面。若素材不足，先提炼二级要点、检查项、责任项、风险项、追踪项，而不是拉大留白。
...
Standard PPT icons MUST be selected from the bundled SVG icon libraries under `templates/icons/` and embedded as vector paths during `finalize_svg.py`. They do not use `image_gen.py`, `gpt-image-2`, `qwen-image-2.0-pro`, or any other image model.
...
```bash
ls skills/PPT-goldwind/templates/icons/chunk/ | grep home
ls skills/PPT-goldwind/templates/icons/tabler-filled/ | grep home
ls skills/PPT-goldwind/templates/icons/tabler-outline/ | grep chart
```
