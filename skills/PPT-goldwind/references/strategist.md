...
|--------------|---------|-----------|
| Project creation + Template option confirmed | **Strategist**: Eight Confirmations + Design Spec + proactive image asset plan | Image_Generator or Executor |
...
2. Read `manifest.json`, `master_layout_refs.json`, and `master_layout_analysis.md` before writing `design_spec.md`. Native PPTX geometry, placeholders, layout inheritance, image relationships, and rotation are the source of truth.
3. Read every SVG listed in `reference_svg_selection.json` when available, but only for visual rhythm/composition cross-checking. If SVG geometry conflicts with native PPTX data, native PPTX data wins.
4. Add a template-mimic contract to the design spec covering page-type mapping, exact reusable anchors, title hierarchy, fonts, and excluded non-template assets.
5. Lock the delivery mode for `金风通用模板`: the final editable PPTX must be generated from the native PowerPoint base `templates/layouts/金风通用模板/goldwind_native_base.pptx` via `scripts/goldwind_native_deck.py`. SVG templates may be used as visual references or fallback previews, but not as the primary Goldwind export.
6. Lock first/last page behavior for `金风通用模板`: cover text may vary only by title, author/name, and date; if no user title is provided, auto-generate it from source content. Ending-page structure must match the native Goldwind cover/ending layout as editable/native elements; the default ending copy may be changed only when the user requests different wording.
7. Do not promote content-specific figures from the reference deck as template assets. In particular, the simulation/arrow figure is excluded.
8. The bottom-right three-stripe page-number block at `x=1204, y=620` is explicitly excluded from `金风通用模板`; do not include it in anchors, placeholders, or page mapping.
9. Lock the background and rail anchors from the reference deck: dotted wave is a full-width layer for cover/ending and wave-background pages; the final PPTX side copyright rail must be a native rotated object (`x=-2.376in y=3.371in w=5.512in h=0.76in rotation=270`, `font-size=8`). SVG fallback may keep `matrix(0 -1.33 1.33 0 40.71 624.67)`, but the final Goldwind output must not rely on SVG matrix export.
10. Keep the Goldwind TOC page to four primary entries only and use the native left-image layout: `toc_wind_left.png` at `x=0 y=-0.006in w=6.92in h=7.506in`, with the right text box at `x=7.653in y=0.187in w=4.899in h=6.614in`. Do not add subtitles/descriptions to TOC rows; those belong in chapter/content pages, not the reference TOC structure.
11. Content-page titles must be planned as native top title placeholder values, not as extra overlay text boxes. Duplicate titles or visible placeholder prompt text are blocking defects.
12. Lock final delivery packaging: expose exactly one PPTX with a Chinese filename derived from the deck title. QA decks, preview images, SVG decks, design docs, scripts, and logs are internal artifacts unless the user explicitly asks for them.
13. Plan Goldwind正文页 for realistic high information density on 100-inch projectors: default each content slide to 10-18 useful business information units. Density must be expressed through varied business layouts, not repeated card grids. For each content page, explicitly set a layout type: `process_flow` for workflows, `timeline` for schedules, `risk_matrix` for risks, `kpi_dashboard` for metrics, `comparison_table` for comparisons, `image_explain` for generated-image explanation pages, `section_summary` for grouped conclusions, and `dense_cards` only for genuinely flat independent item lists. Include conclusions, indicators, actions, owners, risks, dependencies, and next steps where relevant. Do not create sparse slides with only 2-5 bullets unless the source is truly limited or the user requests a breathing page.
14. Add native validation to the project test plan: `python3 scripts/goldwind_native_check.py <output.pptx>` and `python3 scripts/pptx_visibility_check.py <output.pptx>` are blocking final checks.
...
**Icon generation policy (hard rule)**: Standard PPT icons are vector assets from `templates/icons/`; they do not use `image_gen.py` and do not use any image model. `image_gen.py` defaults to the `image2` / `gpt-image-2` backend and is reserved for raster images such as backgrounds, photos, illustrations, decorative patterns, large diagram-style images, visually rich process diagrams, table-to-infographic treatments, and technical architecture visuals. Only classify an icon as AI-generated when the user explicitly asks for a custom icon or when all approved libraries lack a required metaphor after documented search; then record it as an image asset, including the active backend/model from `image_gen.py --list-backends` or environment configuration.
...
> 2. Search for icon availability: `ls skills/PPT-goldwind/templates/icons/<chosen-library>/ | grep <keyword>`
...
| **C** | AI-generated with `gpt-image-2` | Custom illustrations, backgrounds, visual process/architecture/table treatments needed |
...
**When B/C/D is selected, or when the Strategist proactively decides an image asset will improve the deck**, add an image resource list to the spec:
...
### Proactive gpt-image-2 Selection Rules

After the user confirms the Eight Confirmations, actively inspect every page in the content outline and mark image-worthy assets as `Pending generation`. The user does not need to manually request each generated image. However, proactive does not mean vague: every generated image must have enough business content to be worth a slide.

Use `gpt-image-2` proactively for:

| Content pattern | Preferred asset treatment |
|-----------------|---------------------------|
| Cover / chapter opener | Full-bleed or atmospheric background with text-safe negative space |
| Dense table with categories, priorities, risks, comparison, or roadmap rows | Table-to-infographic background, matrix board, or visual summary panel; exact text remains editable overlay |
| Process / workflow / lifecycle | Large process visual, swimlane, pipeline, or scene-based flow background |
| System / product / solution architecture | Technical architecture visual with modules, data flow, infrastructure, or control layers |
| Mechanism / principle / engineering concept | Exploded view, conceptual cutaway, or simplified technical illustration |
| Strategy framework / capability map / risk-control map | Matrix, layered map, hub-spoke, or control-tower style illustration |
| Scenario / user journey / operational scene | Professional scenario illustration or photorealistic scene |

Do not use `gpt-image-2` for:

| Content pattern | Better treatment |
|-----------------|------------------|
| Exact charts with numeric data | Use editable SVG/chart templates; optionally add generated background only |
| Simple KPI cards, bullets, small icons | Use native text, shapes, and bundled SVG icons |
| Tables where every cell must remain editable and auditable | Build native/SVG table; optionally use generated visual header/background |
| Standard process-step icons | Use the selected icon library |

Content-rich image rule: every `Pending generation` item must include:
- business subject and scenario, not just a design style
- 5-10 named modules, stages, objects, or status categories
- relationship logic such as sequence, hierarchy, dependency, risk path, data flow, or ownership
- the editable overlay plan: which Chinese labels, KPI values, callouts, and conclusions will be added in PPT/SVG after generation

If the source material does not provide those details, ask the user one concise question before generation: "这张图需要重点呈现哪些对象、阶段、指标或业务场景？" Do not generate a hollow framework.

Accuracy rule: exact figures, Chinese labels, table cell values, legends, and footnotes must remain editable SVG/PPT text unless the user explicitly accepts a raster-only visual. Generated images should carry business scene detail, module shapes, object relationships, status patterns, visual hierarchy, and domain-specific artifacts; the Executor overlays precise text/data as editable elements.

Prompt safety rule: when adding any `Pending generation` image item, include negative constraints against fake readable text, pseudo letters, random numbers that imply facts, UI gibberish, fake chart values, watermarks, logos, and tiny illegible captions. Do not ask the image model to render final Chinese labels or exact numeric values inside the image. Do allow rich visual content such as process lanes, status dots, wind turbines, maps, equipment, dashboards without text, data traces, matrix cells, component boxes, callout regions, and realistic scenario objects.

Goldwind template style rule: for `金风通用模板`, the generated image description must lock the visual style to the native template: mostly white / very light gray canvas, thin navy and teal linework, pale-gray panels, small teal accent blocks, restrained engineering information-graphic style, and clean overlay-safe areas. Explicitly avoid dark command-center dashboards, neon cyberpunk glow, cinematic black backgrounds, oversized blue gradients, and dense sci-fi UI screens. Avoid wording such as "empty zones", "spacious empty zones", or "blank label areas" unless there is an explicitly planned PPT overlay; these phrases tend to produce empty shell images.
...
> **Pipeline handoff**: When the Image Resource List contains any `Pending generation` asset, auto-proceed to Image_Generator immediately after writing the design spec and `spec_lock.md`. Once images are collected in `images/`, proceed to Executor. Do not ask the user to invoke Image_Generator manually.
...
Next step:
- Image Resource List has `Pending generation` assets → Auto-run Image_Generator with `image2` / `gpt-image-2`
- No generated raster assets needed → Invoke Executor
```
...
Next step:
- Image Resource List has `Pending generation` assets → Auto-run Image_Generator with `image2` / `gpt-image-2`
- No generated raster assets needed → Invoke Executor (free design for every page)
```
