...
Backend selection (`IMAGE_BACKEND` in `.env` or the current process environment):
  IMAGE_BACKEND=image2      -> GPT Image 2 via Codex provider config (default)
...
BACKEND_REGISTRY = {
    ...
    "image2": {
        "module": "backend_image2",
        "tier": "core",
        "label": "GPT Image 2 (Codex provider)",
        "default_model": "gpt-image-2",
        "key_hint": "~/.codex/auth.json OPENAI_API_KEY + config.toml provider base_url",
        "aliases": ["gpt-image-2", "gpt_image_2", "image-2", "image2-provider"],
    },
    "openai": {
...
def _resolve_backend() -> tuple[object, str]:
    ...
    print(
        ...
        "\n"
        "Set IMAGE_BACKEND explicitly in one of these places, or use --backend image2:\n"
...
def main() -> None:
    ...
    parser.add_argument(
        "--aspect_ratio", default="16:9", choices=ALL_ASPECT_RATIOS,
        help=f"Aspect ratio. Default: 16:9."
    )
    parser.add_argument(
        "--image_size", default="2K",
        help=f"Image size. Choices: {ALL_IMAGE_SIZES}. Default: 2K. (case-insensitive)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Override IMAGE_BACKEND env var. Defaults to image2 when IMAGE_BACKEND is unset."
    )
    parser.add_argument(
    ...
    elif not os.environ.get("IMAGE_BACKEND"):
        os.environ["IMAGE_BACKEND"] = "image2"
