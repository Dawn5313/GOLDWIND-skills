...
def load_minimal_toml(path: Path) -> dict:
    """
    Parse the small TOML subset needed from Codex config files on Python < 3.11.
    ...
    for raw_line in path.read_text(encoding="utf-8").splitlines():
        ...
        if line.startswith("[") and line.endswith("]"):
            current = root
            for part in split_minimal_toml_path(line[1:-1]):
                key = parse_minimal_toml_key(part)
                if not key:
                    raise ValueError(f"Invalid TOML table in {path}: {raw_line}")
                current = current.setdefault(key, {})
            continue
        if "=" not in line:
            continue
        key, value = line.split("=", 1)
        current[key.strip()] = parse_minimal_toml_value(value.strip())
    return root


def split_minimal_toml_path(value: str) -> list[str]:
    parts: list[str] = []
    current: list[str] = []
    quote = ""
    for char in value:
...
def parse_minimal_toml_key(value: str) -> str:
    value = value.strip()
...
def parse_minimal_toml_value(value: str):
    if len(value) >= 2 and value[0] == value[-1] and value[0] in ("'", '"'):
        return value[1:-1]
    lowered = value.lower()
    if lowered == "true":
...
def load_provider_config(
    ...
    if base_url is None:
        ...
        base_url = resolve_image_base_url(provider.get("base_url"), config.get("openai_base_url"))
...
def resolve_image_base_url(provider_base_url: object, openai_base_url: object) -> object:
    """Choose the best image API base URL from Codex config variants."""
    if isinstance(provider_base_url, str) and image_url_has_api_path(provider_base_url):
...
def image_url_has_api_path(base_url: str) -> bool:
    parsed = urlparse(base_url.strip())
    path = parsed.path.rstrip("/")
    return path.endswith("/v1") or path.endswith("/v1beta")
