...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description='PPT Goldwind - SVG to PPTX Tool (Office Compatibility Mode)',
...
''',
    ...
    if gen_native:
        if verbose:
            print("PPT Goldwind - SVG to PPTX Tool")
            print("=" * 50)
    ...
    if gen_legacy:
        if verbose:
            if gen_native:
                print()
                print("-" * 50)
            print("PPT Goldwind - SVG to PPTX Tool (SVG Reference)")
            print("=" * 50)
            print(f"  Project path: {project_path}")
            print(f"  SVG directory: {source_dir_name}")
